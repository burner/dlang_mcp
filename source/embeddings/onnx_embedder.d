...
class OnnxEmbedder : Embedder
{
    ...
    private bool tryLoadModel()
    {
        ...
        ort.SetSessionGraphOptimizationLevel(sessionOptions, GraphOptimizationLevel.ORT_ENABLE_BASIC);
        ...
        scope(exit) freeORTCharPath(modelPathORT);
        
        status = ort.CreateSession(env, cast(const(wchar_t)*)modelPathORT, sessionOptions, &session);
        if (status !is null)
        {
            stderr.writeln("Failed to create ONNX session: ", ort.GetErrorMessage(status).to!string());
            ort.ReleaseStatus(status);
            ort.ReleaseSessionOptions(sessionOptions);
            ort.ReleaseEnv(env);
            env = null;
    ...
    float[] embed(string text)
    {
        ...
        status = ort.CreateCpuMemoryInfo(OrtAllocatorType.OrtArenaAllocator, OrtMemType.OrtMemTypeDefault, &memoryInfo);
        if (status !is null)
        {
            ort.ReleaseStatus(status);
            return fallback.embed(text);
        ...
        OrtValue*[] inputTensors = [inputIdsTensor, attentionMaskTensor, tokenTypeIdsTensor];
        ...
        status = ort.Run(session, null, inputNames.ptr, inputTensors.ptr, 3, outputNames.ptr, 1, &outputTensor);
