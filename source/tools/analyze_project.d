...
class AnalyzeProjectTool : BaseTool
{
    ...
    ToolResult execute(JSONValue arguments)
    {
        try
        {
            ...
            {
                return createTextResult(formatMinimalProject(projectPath, "dub.sdl found but SDL parsing is limited"));
            }

            return createTextResult(formatMinimalProject(projectPath, "No dub.json or dub.sdl found"));
        }
        catch (Exception e)
        {
...
private:
    ...
    void formatPackageInfo(ref typeof(appender!string()) output, JSONValue pkg, string projectPath)
    {
        ...
        {
            output ~= "Direct dependencies: " ~ pkg["dependencies"].array.map!(d => d.str).join(", ") ~ "\n";
        }
        ...
        {
            output ~= "Version identifiers: " ~ pkg["versions"].array.map!(v => v.str).join(", ") ~ "\n";
        }
